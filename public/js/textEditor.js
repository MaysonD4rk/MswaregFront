        ...
        if(ideaInfo.content.length >= 500){
            document.cookie.split(';').forEach(async cookie => {
                ...
                if (authToken[0] == ' authToken' || authToken[0] == 'authToken') {
                    try {
                        var pub = await axios.post('http://192.168.2.106:8000/pub',{
                    ...
                    headers: {
                        ...
                        window.location.href = `http://192.168.2.106:8080/addPubImg/${pub.data[0]}`;
                    } catch (error) {
...
async function confirmEditIdea() {
    ...
    if (ideaInfo.content.length >= 500) {
        document.cookie.split(';').forEach(async cookie => {
            ...
            if (authToken[0] == ' authToken' || authToken[0] == 'authToken') {
                try {
                    var pub = await axios.put('http://192.168.2.106:8000/pub', {
                        pubId,
                        title: inputTitle.value,
                        ideaSummary: inputSummary.value,
                        mainIdea: ideaInfo.content,
                    ...
                    window.location.href = `http://192.168.2.106:8080/addPubImg/${pubId}`;
                } catch (error) {
                    console.log(error)
                }
