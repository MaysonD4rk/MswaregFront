...
if (!!document.getElementById('search')) {
    ...
    document.getElementById('search').addEventListener('keydown', async (e) => {
        ...
        if (document.getElementById('search').value.length > 0) {
            ...
            try {
                const results = await axios.get('http://localhost:8000/generalSeach/' + currentSearchValue);
                renderCards(results)
        ...
        if (queryInput != '') {
            if (queryType != '' && queryType == 'userQuery') {
                if ((document.getElementsByTagName('body')[0].scrollHeight - window.scrollY) <= 761) {
                    ++offsetUsersSearch
                    try {
                        let users = await axios.get(`http://localhost:8000/getSearchListUser/${offsetUsersSearch * 20}/${queryInput}`);
                ...
                if ((document.getElementsByTagName('body')[0].scrollHeight - window.scrollY) <= 761) {
                    ++offsetMsgsSearch
                    try {
                        let msgs = await axios.get(`http://localhost:8000/searchForMsg/${offsetMsgsSearch * 15}/${queryInput}`);
