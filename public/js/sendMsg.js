...
textTo.addEventListener('keydown', (event) => {
    ...
    axios.get('http://192.168.2.106:8000/searchUser/' + document.getElementById('textTo').value)
        .then((res) => {
...
function searchMsg() {
    ...
    axios.get('http://192.168.2.106:8000/searchMsgList/0/' + document.getElementById('search').value)
        .then(msgs => {
...
async function writeMsg(senter) {
    ...
    document.cookie.split(';').forEach(async cookie => {
        ...
        if (authToken[0] == ' authToken' || authToken[0] == 'authToken') {
            ...
            try {
                let tryWrite = await axios.post('http://192.168.2.106:8000/writeMsg', { userId: senter, recipientName: recipient, msg }, {
                    headers: {
