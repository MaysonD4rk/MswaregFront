...
async function openIdeaModal(id){
    ...
    let result = await axios.get(`http://192.168.2.104:8000/findPub/${id}`);
    ...
    document.getElementsByClassName('actionButton')[0].onclick = async ()=>{
        if (donateTable) {
            ...
            var data = await axios.get(`http://192.168.2.104:8000/listDonates/${id}`);
            console.log(data);
...
async function confirmPurchase(userId){
    ...
    document.cookie.split(';').forEach(async cookie => {
        ...
        if (authToken[0] == ' authToken' || authToken[0] == 'authToken') {
            ...
            try {
                var result = await axios.post('http://192.168.2.104:8000/donateCredits', {
                    userId,
                    credits,
...
async function openModal(elementId, itemId, allowFeedback=true){
    ...
    let result = await axios.get(`http://192.168.2.104:8000/findPub/${itemId}`);
    ...
    document.getElementsByClassName('sendFeedback')[0].onclick = async ()=>{
        document.cookie.split(';').forEach(async cookie => {
            ...
            if (authToken[0] == ' authToken' || authToken[0] == 'authToken') {
                try {
                    let sendFeedbackResult = await axios.post('http://192.168.2.104:8000/sendFeedback', {
                        userId,
                        ideaId: itemId,
    ...
    document.getElementsByClassName('sendReport')[0].onclick = async () => {
        document.cookie.split(';').forEach(async cookie => {
            ...
            if (authToken[0] == ' authToken' || authToken[0] == 'authToken') {
                ...
                try {
                    let sendReportResult = await axios.post('http://192.168.2.104:8000/sendReport', {
                        userId,
...
async function openInfoModal(userId){
    try {
        const userInfo = await axios.get('http://192.168.2.104:8000/userHelpInfo/'+userId)
...
async function getInvestment(investmentId){
    try {
        const investmentData = await axios.get('http://192.168.2.104:8000/getOneDonate/'+investmentId);
