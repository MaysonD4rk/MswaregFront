(async function verifyNotifications(){
    try {
        const notifications = await axios.get("http://192.168.2.106:8000/verifyActiveNotifications/"+document.getElementById('userId').value);
    ...
    if (newPass == confirmNewPass) {
        ...
        document.cookie.split(';').forEach(async cookie => {
                ...
                if (authToken[0] == ' authToken' || authToken[0] == 'authToken') {
                    try {
                        var verifyOldPass = await axios.post('http://192.168.2.106:8000/login', {
                            email: email,
                            password: oldPass
                        })
                        ...
                        if (verifyOldPass.status == 200) {
                            ...
                            try {
                                var updatePass = await axios.put('http://192.168.2.106:8000/updatePass', {
                                    userId: userId,
                                    email: email,
                                    password: newPass
                                }, {
...
async function getTokenUpdateInfo(){
    ...
    document.cookie.split(';').forEach(async cookie => {
                ...
                if (authToken[0] == ' authToken' || authToken[0] == 'authToken') {
    try {
        let updateInfoCode = await axios.post('http://192.168.2.106:8000/updateInfoToken', {
            email,
            userId
        }, {
            headers: {
...
async function updateInfo() {
    ...
    document.cookie.split(';').forEach(async cookie => {
                ...
                if (authToken[0] == ' authToken' || authToken[0] == 'authToken') {
    try {
        let updateInfo = await axios.put('http://192.168.2.106:8000/updateInfo', {
            FirstName,
            LastName,
            pixKey,
            userId,
...
async function updateNotification() {
    ...
    document.cookie.split(';').forEach(async cookie => {
                ...
                if (authToken[0] == ' authToken' || authToken[0] == 'authToken') {
            try {
                const updateNotification = await axios.put('http://192.168.2.106:8000/updateNotifications', {
                    notification1,
                    notification2,
                    notification3,
                    notification4,
...
document.getElementById('changeUsername').onclick = async ()=>{
    ...
    document.cookie.split(';').forEach(async cookie => {
                ...
                if (authToken[0] == ' authToken' || authToken[0] == 'authToken') {
    try {
        const changeUsername = await axios.put('http://192.168.2.106:8000/changeUsername',{
            userId:userId.value,
            username
        }, {
            headers: {
...
async function updateCurrentCode(){
    document.cookie.split(';').forEach(async cookie => {
        ...
        if (authToken[0] == ' authToken' || authToken[0] == 'authToken') {
            try {
                const updateCode = await axios.put('http://192.168.2.106:8000/updatePersonalCode', {
