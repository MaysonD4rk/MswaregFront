async function changePass(email) {
    ...
    if (newPass == confirmNewPass) {
        ...
        try {
            var verifyOldPass = await axios.post('http://15.228.160.53:8000/login', {
                email: email,
                password: oldPass
            })
            ...
            if (verifyOldPass.status == 200) {
                ...
                try {
                    var updatePass = await axios.put('http://15.228.160.53:8000/updatePass', {
                        email: email,
                        password: newPass
                    })
...
async function updateInfo() {
    ...
    try {
        let updateInfo = await axios.put('http://15.228.160.53:8000/updateInfo', {
            FirstName,
            LastName,
            userId
        })
...
async function updateNotification() {
    ...
    try {
        const updateNotification = await axios.put('http://15.228.160.53:8000/updateNotifications', {
            notification1,
            notification2,
            notification3,
            notification4,
